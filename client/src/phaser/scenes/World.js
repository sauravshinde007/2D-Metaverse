...
export default class WorldScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.positionUpdateInterval = setInterval(() => {
            ...
            if (currentState.x !== this.lastSentState.x || currentState.y !== this.lastSentState.y || currentState.anim !== this.lastSentState.anim) {
                this.socket.emit("move", currentState);
                this.lastSentState = currentState;
            }
        }, 100);

        // --- YOUR PREVIOUS OVERLAY LOGIC - RESTORED ---
    ...
    update() {
        ...
        if (this.player.body.velocity.x < 0) {
            this.currentAnimation = "walk-left"; this.lastDirection = "left";
        } else if (this.player.body.velocity.x > 0) {
            this.currentAnimation = "walk-right"; this.lastDirection = "right";
        } else if (this.player.body.velocity.y < 0) {
            this.currentAnimation = "walk-up"; this.lastDirection = "up";
        } else if (this.player.body.velocity.y > 0) {
            this.currentAnimation = "walk-down"; this.lastDirection = "down";
        } else {
    ...
    // Using shutdown() for proper scene cleanup in Phaser
    shutdown() {
